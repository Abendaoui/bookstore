...
import { MoreIcon, Title, SaveIcon, Breadcrumb } from "../components";
...
const Transactions = () => {
    ...
    const getTransactions = async (page = 1) => {
        await axiosClient
            .get(`/gettransactions/${chosenCustomer}?page=${page}`, {
    ...
    React.useEffect(() => {
        getTransactions();
    }, [chosenCustomer, displayCount]);
    return (
        <section>
            <Breadcrumb />
            <Title>List Of Complete Transactions</Title>
            {/* Filter Section */}
            ...
            <div className="rounded-lg border border-gray-200 shadow-lg m-5">
                <table className="w-full border-collapse bg-white text-left text-sm text-gray-500 rounded-xl">
                    ...
                    <tbody className="divide-y divide-gray-100 border-t border-gray-100">
                        {transactions.map((transaction) => {
                            return (
                                <Row key={transaction.id} {...transaction} />
                            );
                        })}
                    </tbody>
                </table>
                ...
                <div className="flex items-center justify-between border-t border-gray-200 bg-gray-100 px-4 py-3 sm:px-6">
                    <Pagination
                        ...
                        onChange={(page) => getTransactions(page)}
...
const Row = ({
    ...
    return (
        <tr className="hover:bg-gray-50">
            ...
            <td className={`px-6 py-4`}>{getOrderStatus(status)}</td>
            ...
            <td className="px-6 py-4">
                <div className="flex justify-center gap-4">
                    <MoreIcon link={`/dashboard/transactions/${id}`} />
                    <SaveIcon save={() => console.log("save")} />
                </div>
            </td>
        </tr>
    );
